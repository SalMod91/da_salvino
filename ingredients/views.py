...
class IngredientsPage(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # For each category, filter the ingredients from the database
        # that belong to that category and store them in the
        # categorized_ingredients dictionary with the category name as the key
        for category in categories:
            ingredients = Ingredient.objects.filter(
                    category=category
                ).order_by('name')  # Orders ingredients alphabetically by name
        ...
        # Returns the context dictionary wich now includes
        # the 'categorized_ingredients'
        return context
