        ...
        if 'submit_login' in request.POST:
            ...
            if login_form.is_valid():
                ...
                if user is not None:
                    if user.is_approved:
                        login(request, user)
                        # Future message saying that logged in
                        return redirect('staff_portal') 
                    else:
                        pass
                        # future message saying waiting for approval
            else:
                signup_form = StaffUserCreationForm()
            ...
            if signup_form.is_valid():
                signup_form.save()
                # future message saying that registration was succesfull
                return redirect('staff_portal')
            else:
                login_form = AuthenticationForm()
            ...
            logout(request)
            # Future message saying that succesfully logged out
            return redirect('staff_portal')
